...
module.exports.execute = async (client, message, args, ayar, emoji) => {
  ...
  if(kisi) {
    ...
    if(ayar.banLogKanali && client.channels.has(ayar.banLogKanali)) client.channels.get(ayar.banLogKanali).send(new RichEmbed().setColor(client.randomColor()).setTimestamp().setFooter("YASHINU ❤️ ALOSHA").setTitle('Ban Kaldırıldı!').setDescription(`**Kaldıran Yetkili:** ${message.author} (${message.author.id})\n**Banı Kaldırılan Üye:** ${kisi.tag} (${kisi.id})`));
  } else {
