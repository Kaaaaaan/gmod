...
module.exports.onLoad = async (client) => {
    ...
    client.on("message", async message => {
      ...
      if ((ayarlar.teyitsizRolleri && ayarlar.teyitsizRolleri.some(rol => message.member.roles.has(rol))) || (ayarlar.jailRolu && message.member.roles.has(ayarlar.jailRolu))) return;
...
module.exports.execute = async (client, message, args, ayar, emoji) => {
    ...
    let embed = new RichEmbed().setThumbnail(user.user.avatarURL({dynamic: true, size: 2048})).setAuthor(user.displayName, user.user.avatarURL({dynamic: true, size: 2048})).setFooter("YASHINU ❤️ ALOSHA").setColor(client.randomColor()).setTimestamp();
