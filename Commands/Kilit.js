...
module.exports.execute = async(client, message, args, ayar, emoji) => {
  ...
  if(!message.member.roles.has(ayar.sahipRolu)) return message.channel.send(embed.setDescription("Bu komutu kullanabilmek için gerekli rollere sahip değilsin!")).then(x => x.delete({timeout: 5000}));
  let everyone = message.guild.roles.find(a => a.name === "@everyone");
  let permObjesi = {};
