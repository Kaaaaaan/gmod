...
module.exports.execute = async(client, message, args, ayar, emoji) => {
  let guild = message.guild;
    ...
    if(!conf.sahip.some(x => message.author.id == x) && !message.member.roles.has(ayar.sahipRolu)) return;
  ...
  if(args[0] === "sil" || args[0] === "kaldır") {
    ...
    let emojisim = guild.emojis.find(e => e.name === link);
  ...
  if(args[0] === "id") {
    try {
      message.channel.send(`Sunucuda Bulunan Emojiler (${message.guild.emojis.size} adet) \n\n${message.guild.emojis.map(emoji => emoji.id + " | " + emoji.toString()).join('\n')}`, {code: 'xl', split: true})
    } catch (err) { };
    return
  };
  ...
  if (message.guild.emojis.some(x => `${x.name}`.includes(args[0]))) {
    ...
    const emoji = new RichEmbed()
    .setColor('RANDOM')
    .setTimestamp()
    .setFooter(client.user.username, client.user.avatarURL)
    .setDescription(`**Emoji:**  ${message.guild.emojis.find(a => a.name === args[0])} \n**Emoji Adı:**  ${message.guild.emojis.find(a => a.name === args[0]).name} \n**Emoji ID'si:**  ${message.guild.emojis.find(a => a.name === args[0]).id} \n**Emoji Kodu:**  \`${message.guild.emojis.find(x => x.name == args[0]).toString()}\``);
    try {
