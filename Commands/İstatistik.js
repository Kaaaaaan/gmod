...
module.exports.execute = async(client, message, args, ayar, emoji) => {
  let kullanici = message.mentions.users.first() || client.users.get(args[0]) || (args.length > 0 ? client.users.filter(e => e.username.toLowerCase().includes(args.join(" ").toLowerCase())).first(): message.author) || message.author;
  let uye = message.guild.member(kullanici);
  
  let guild = message.guild;
  let yetkiliBilgisi = ``;
  if((ayar.sahipRolu && uye.roles.has(ayar.sahipRolu)) || (ayar.teyitciRolleri && ayar.teyitciRolleri.some(rol => uye.roles.has(rol)))) {
  ...
  if((ayar.sahipRolu && uye.roles.has(ayar.sahipRolu)) || (ayar.muteciRolleri && ayar.muteciRolleri.some(rol => uye.roles.has(rol))) || (ayar.banciRolleri && ayar.banciRolleri.some(rol => uye.roles.has(rol))) || (ayar.banciRolleri && ayar.banciRolleri.some(rol => uye.roles.has(rol))) || (ayar.jailciRolleri && ayar.jailciRolleri.some(rol => uye.roles.has(rol)))) {
  ...
  const embed = new RichEmbed().setTimestamp().setColor(client.randomColor()).setFooter(message.guild.name, message.guild.iconURL({dynamic: true, size: 2048})).setAuthor(kullanici.tag.replace("`", ""), kullanici.avatarURL({dynamic: true, size: 2048})).setThumbnail(kullanici.avatarURL({dynamic: true, size: 2048}))
  ...
  .addField(`__**Üyelik Bilgisi**__`, `\`Takma Adı:\` ${uye.displayName.replace("`", "")} ${uye.nickname ? "" : "[Yok]"}\n\`Katılma Tarihi:\` ${moment(uye.joinedAt).format(`DD/MM/YYYY | HH:mm`)}\n\`Katılım Sırası:\` ${(message.guild.members.filter(a => a.joinedTimestamp <= uye.joinedTimestamp).size).toLocaleString()}/${(message.guild.memberCount).toLocaleString()}\n\`Rolleri:\` ${uye.roles.size <= 5 ? uye.roles.filter(x => x.name !== "@everyone").map(x => x).join(', ') : `Listelenemedi! (${uye.roles.size})`}\n${yetkiliBilgisi}`);  
  let raw = sdb.get(`raw`);
  ...
  console.log(mostVoice)
        ...
        embed.addField(`En Aktif Kanalları`, `**   ** ${mostText == undefined ? "" : `Mesaj: ${client.channels.has(mostText) ? client.channels.get(mostText) : "#deleted-channel"}: \`${numberWithCommas(messageData[raw.day][message.author.id][mostText] || 0)} mesaj\``} \n ${mostVoice == undefined ? "" : `Ses: ${client.channels
          
          
          .has(mostVoice) ? client.channels.get(mostVoice).name : "#deleted-channel"}: \`${moment.duration(explodeData[raw.day][message.author.id][mostVoice]).format("H [saat,] m [dakika]")}\``}`)
