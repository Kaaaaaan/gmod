...
module.exports.execute = async (client, message, args) => {
  ...
  let secim = args[0];
  const embed = new RichEmbed().setColor(client.randomColor()).setAuthor(message.member.displayName, message.author.avatarURL({ dynamic: true })).setFooter("YASHINU ❤️ ALOSHA");
  if (secim == "liste") {
    ...
    let ozelliklerListe = Object.keys(data || {}).filter(a => ozellikler.find(v => v.name == a)).map(o => {
      ...
      else if(ozellik.type == "roller") return `\`${o}\` - ${element.map(role => message.guild.roles.get(role)).join(', ')}`
      else if(ozellik.type == "rol") return `\`${o}\` - ${message.guild.roles.get(element) || "Rol bulunamadı!"}`
      else if(ozellik.type == "kanal") return `\`${o}\` - ${message.guild.channels.get(element) || "Kanal bulunamadı!"}`
  ...
  if (ozellik.type == "tekil"){
    ...
    else roller = args.splice(1).filter(role => message.guild.roles.some(role2 => role == role2.id));
    ...
    return message.channel.send(embed.setDescription(`Belirttiğin ayar için ${roller.map(role => message.guild.roles.filter(role2 => role == role2.id).map(role => role.toString())).join(", ")} tanımladın.`))
  }
  else if (ozellik.type == "rol") {
    let rol = message.mentions.roles.first() || message.guild.roles.get(args.splice(1)[0]) || message.guild.roles.find(r => r.name === args.splice(1).join(' '));
    ...
    let channel = message.guild.channels.get(args.splice(1)[0]) || message.mentions.channels.first();
