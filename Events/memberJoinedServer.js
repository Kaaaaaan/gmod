...
module.exports = async (member) => {
  ...
  if(jaildekiler.some(x => x.includes(member.id)) || tempjaildekiler.some(x => x.id === member.id)){
    ...
    if(ayarlar.fakeHesapLogKanali && member.guild.channels.has(ayarlar.fakeHesapLogKanali)) return member.guild.channels.get(ayarlar.teyitKanali).send(new MessageEmbed().setAuthor(member.guild.name, member.guild.iconURL({dynamic: true})).setDescription(`${member} üyesi sunucuya katıldı fakat hesabı ${member.client.tarihHesapla(member.user.createdAt)} açıldığı için jaile atıldı!`).setTimestamp().setFooter("YASHINU ❤️ ALOSHA"));
  ...
  **${client.emoji("gif4")} • Hesap ${guvenilirlik ? "Tehlikeli!" : "Güvenli!"}**
  ...
  if(ayarlar.teyitKanali && member.guild.channels.has(ayarlar.teyitKanali)) member.guild.channels.get(ayarlar.teyitKanali).send({ embed: embed }); 
}
