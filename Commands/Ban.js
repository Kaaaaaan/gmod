...
module.exports.execute = async (client, message, args, ayar, emoji) => {
  ...
  if (!victim) {
    ...
    if(kisi) {
      ...
      if(ayar.banLogKanali && client.channels.has(ayar.banLogKanali)) client.channels.get(ayar.banLogKanali).send(new RichEmbed().setColor(client.randomColor()).setTimestamp().setFooter("YASHINU ❤️ ALOSHA").setTitle('Üye Banlandı!').setDescription(`**Banlayan Yetkili:** ${message.author} (${message.author.id})\n**Banlanan Üye:** ${kisi.tag} (${kisi.id})\n**Sebep:** ${reason}`));
    } else {
