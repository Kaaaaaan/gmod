...
module.exports.execute = async (client, message, args, ayar, emoji) => {
  ...
  if(!ayar.teyitciRolleri.some(rol => message.member.roles.has(rol)) && !message.member.roles.has(ayar.sahipRolu)) return message.channel.send(embed.setDescription(`İsim komutunu kullanabilmek için herhangi bir yetkiye sahip değilsin.`)).then(x => x.delete({timeout: 5000}));
  ...
  message.channel.send(new RichEmbed().setColor(client.randomColor()).setTitle("Kayıt Sistemi").setThumbnail(uye.user.avatarURL({dynamic: true, size: 2048})).setFooter("YASHINU ❤️ ALOSHA").setTimestamp().setDescription(`${emoji("gif1")} Kayıt Olan Kullanıcı: ${uye}\n${emoji("gif1")} Verilen Roller: ${uye.roles.filter(a => a.name !== "@everyone").map(x => x).join(', ')}\n${emoji("gif1")} Kayıt Eden: ${message.author}\n${emoji("gif1")} Güncellenen İsim: \`${yazilacakIsim}\``)).catch();
};
