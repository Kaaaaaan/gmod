...
module.exports.execute = async (client, message, args, ayar, emoji) => {
  ...
  if(!ayar.jailciRolleri.some(rol => message.member.roles.has(rol)) && !message.member.roles.has(ayar.sahipRolu)) return message.channel.send(embed.setDescription(`Jail komutunu kullanabilmek için herhangi bir yetkiye sahip değilsin.`)).then(x => x.delete({timeout: 5000}));
  ...
  await uye.roles.set(uye.roles.has(ayar.boosterRolu) ? [ayar.boosterRolu, ayar.jailRolu] : [ayar.jailRolu]).catch();
  ...
  if(ayar.jailLogKanali && client.channels.has(ayar.jailLogKanali)) client.channels.get(ayar.jailLogKanali).send(embed.setDescription(`${uye} üyesi, ${message.author} tarafından **${sure}** boyunca **${reason}** nedeniyle jaile atıldı!`)).catch();
};
