...
module.exports.execute = async (client, message, args, ayar, emoji) => {
  ...
  if(!ayar.teyitciRolleri.some(rol => message.member.roles.has(rol)) && !message.member.roles.has(ayar.sahipRolu)) return message.channel.send(embed.setDescription(`Kayıt komutunu kullanabilmek için herhangi bir yetkiye sahip değilsin.`)).then(x => x.delete({timeout: 5000}));
  ...
  if (db.get(`ayar.isim-yas`)) {
    ...
    yazilacakIsim = `${uye.user.username.includes(ayar.tag) ? ayar.tag : (ayar.ikinciTag ? ayar.ikinciTag : (ayar.tag || ""))} ${isim}`;
      ...
      if (ayar.teyitsizRolleri && ayar.teyitsizRolleri.some(rol => uye.roles.has(rol))) kdb.add(`teyit.${message.author.id}.erkek`, 1);
