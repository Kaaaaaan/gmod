  ...
  client.on("message", (message) => {
    ...
    if (komutlar && komutlar.length) {
      for (let komut of komutlar) {
        ...
        if (isim) {
          if (komut.karalisteKanallari && komut.karalisteKanallari.length && komut.karalisteKanallari.filter(item => message.guild.channels.has(item)).some(item => message.channel.id == item)) return message.channel.send("Bu komut "+komut.karalisteKanallari.map(e => message.guild.channels.get(e).toString()).join(", ")+" kanal(lar)ında kullanılamaz!");
          if (komut.yetki.kisiler && komut.yetki.kisiler.filter(item => client.users.has(item)).some(e => message.author.id == e)) çalıştır();
          else if (komut.yetki.roller && komut.yetki.roller.filter(item => message.guild.roles.has(item)).some(e => message.member.roles.has(e))) çalıştır();
        ...
        function çalıştır() {
          ...
          let hedef = komut.hedefKisi == 1 ? message.member : (message.mentions.members.first() || message.guild.members.get(argümanlar[0]));
          if (komut.tur == "mesaj") {
            ...
            if (komut.parametreler.verilecekRoller.filter(a => message.guild.roles.has(a)).length) for (let id of komut.parametreler.verilecekRoller) hedef.roles.add(id);
            if (komut.parametreler.alinacakRoller.filter(a => message.guild.roles.has(a)).length) for (let id of komut.parametreler.alinacakRoller) hedef.roles.remove(id);
...
module.exports.execute = async (client, message, args, ayar, emoji) => {
  if(!conf.sahip.some(x => message.author.id == x) && !message.member.roles.has(ayar.sahipRolu)) return message.reply('Bot yapımcılarına ve sunucu sahibine özeldir!').then(x => x.delete({timeout: 5000}));
  ...
  if (args[0] == "ekle") {
    ...
    message.channel.awaitMessages(m => m.author.id == message.author.id, { max: 1, time: 30000, errors: ["time"]})
      ...
      message.channel.awaitMessages(m => m.author.id == message.author.id, { max: 1, time: 30000, errors: ["time"]})
        ...
        else komut.karalisteKanallari = kanallar.first().mentions.channels.filter(a => message.guild.channels.has(a.id)).map(e => e.id).temizle();
        kanallar.first().delete();
    ...
    if (!args[1]) {
      ...
      if (komut.tur == "mesaj") {
        ...
        if (komut.parametreler.verilecekRoller && komut.parametreler.verilecekRoller.length > 0) str += ` | Verilecek roller: ${komut.parametreler.verilecekRoller.filter(a => message.guild.roles.has(a)).length ? komut.parametreler.verilecekRoller.filter(a => message.guild.roles.has(a)).map(e => message.guild.roles.get(e).name).join(", ") : "Yok!"}\n`
        if (komut.parametreler.alinacakRoller && komut.parametreler.alinacakRoller.length > 0) str += ` | Alınacak roller: ${komut.parametreler.alinacakRoller.filter(a => message.guild.roles.has(a)).length ? komut.parametreler.alinacakRoller.filter(a => message.guild.roles.has(a)).map(e => message.guild.roles.get(e).name).join(", ") : "Yok!"}`
      }
      if (komut.yetki.kisiler && komut.yetki.kisiler.length > 0) str2 += `| Kişiler: ${komut.yetki.kisiler.filter(a => client.users.has(a)).length ? komut.yetki.kisiler.filter(a => client.users.has(a)).map(e => client.users.get(e).tag).join(", ").replace(/'/, "").replace(/"/, "") : "Yok!"}\n`;
      if (komut.yetki.roller && komut.yetki.roller.length > 0) str2 += ` | Roller: ${komut.yetki.roller.filter(a => message.guild.roles.has(a)).length ? komut.yetki.roller.filter(a => message.guild.roles.has(a)).map(e => message.guild.roles.get(e).name).join(", ").replace(/'/, "").replace(/"/, "") : "Yok!"}\n`;
...
"Kullanılamayacak kanallar": ${komut.karalisteKanallari && komut.karalisteKanallari.length > 0 ? komut.karalisteKanallari.filter(a => message.guild.channels.has(a)).map(e => message.guild.channels.get(e).name).join(", ") : "Yok!"}
