...
module.exports.execute = async (client, message, args, ayar, emoji) => {
  ...
  if(!ayar.banciRolleri.some(rol => message.member.roles.has(rol)) && !message.member.roles.has(ayar.sahipRolu)) return message.channel.send(embed.setDescription("Bu komutu kullanabilmek için gerekli rollere sahip değilsin!")).then(x => x.delete({timeout: 5000}));
  ...
  if(ayar.banLogKanali && client.channels.has(ayar.banLogKanali)) client.channels.get(ayar.banLogKanali).send(new RichEmbed().setColor(client.randomColor()).setTimestamp().setFooter("YASHINU ❤️ ALOSHA").setTitle('Üye Kicklendi!').setDescription(`**Kickleyen Yetkili:** ${message.author} (${message.author.id})\n**Kicklenen Üye:** ${victim.user.tag} (${victim.user.id})\n**Sebep:** ${reason}`));
};
