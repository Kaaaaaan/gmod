...
module.exports.execute = async (client, message, args, ayar, emoji) => {
  ...
  if(!ayar.muteciRolleri.some(rol => message.member.roles.has(rol)) && !message.member.roles.has(ayar.sahipRolu)) return message.channel.send(embed.setDescription(`Unmute komutunu kullanabilmek için herhangi bir yetkiye sahip değilsin.`)).then(x => x.delete({timeout: 5000}));
  ...
  if(ayar.muteLogKanali && client.channels.has(ayar.muteLogKanali)) client.channels.get(ayar.muteLogKanali).send(embed.setDescription(`${uye} üyesinin, ${message.author} tarafından mutesi kaldırıldı!`)).catch();
};
